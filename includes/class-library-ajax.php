    ...
    foreach ( $blogusers as $user ) {
        ...
        foreach ($check_rents as $check_rent) {
            if ( $check_rent[ID] == $book_[ID] ) {
                ...
                $html[] .= ' - ';    
                $html[] .= '<div class="return_book" style="display: inline;" data="' . $i . '" data-userid="' . $user->ID . '" data-bookid="' . $book_[ID] . '"><i class="material-icons md-24">clear</i></div>';
            }
            $i++;
        }
    }
...
function ajax_return_book($post) {
    ...
    $get_book_quantity_available = get_post_meta(  $object_id['book_ids'], 'meta_book_quantity_available', true );
    ...
    foreach ( $blogusers as $user ) {
        ...
        $i = 0;
        foreach ($check_rents as $check_rent) {
            if ( $check_rent[ID] == $object_id['book_id'] ) {
                $html[] = '</br>';
                $html[] .= $user->ID;
                $html[] .= ' - ';
                $html[] .= $check_rent[ID];
                ...
                $html[] .= "Rent until " . date("m/d/Y h:i:s A T", $check_rent[Time]);
                $html[] .= ' - ';    
                $html[] .= '<div class="return_book" style="display: inline;" data="' . $i . '" data-userid="' . $user->ID . '" data-bookid="' . $object_id['book_id'] . '"><i class="material-icons md-24">clear</i></div>';
            }
            $i++;
        }
    }
